...
export default function Reportage() {
  ...
  const content = {
    FR: { quote: "Certaines histoires n'attendent pas. Certaines vies méritent d'être vues.", intro: "Au-delà du studio et du monde de l'entreprise...", archive: "Archive" },
    EN: { quote: "Some stories don't wait. Some lives deserve to be seen.", intro: "Beyond the studio and the corporate world...", archive: "Archive" }
  };
  ...
  return (
    <main className="min-h-screen bg-white pt-28 md:pt-44 pb-24 px-4 md:px-12 lg:px-20 text-black font-serif">
      <header className="max-w-4xl mx-auto text-left mb-16 md:mb-32">
        <h1 className="text-xl md:text-3xl uppercase tracking-[0.2em] leading-relaxed text-black font-serif">{content[lang].quote}</h1>
        <p className="mt-8 text-[9px] md:text-[10px] uppercase tracking-[0.2em] text-black font-sans leading-loose max-w-2xl">{content[lang].intro}</p>
      </header>
          ...
          if (item.isPlaceholder) {
            return (
              <div key={`empty-${index}`} className={`w-full ${currentRatio} bg-white border border-black/5 flex items-center justify-center`}>
                <span className="text-[7px] uppercase tracking-[0.4em] text-black opacity-20 font-sans">{content[lang].archive}</span>
              </div>
            );
          }
